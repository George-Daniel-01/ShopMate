...
export const fetchAllOrders = catchAsyncErrors(async (req, res, next) => {
  const result = await database.query(`
    ...
    LEFT JOIN order_items oi ON o.id = oi.order_id
    LEFT JOIN shipping_info s ON o.id = s.order_id
    -- ✅ FIXED: Removed "WHERE o.paid_at IS NOT NULL"
    -- That condition was hiding all orders that haven't been paid via Stripe.
    -- Now ALL orders show up in the admin panel regardless of payment status.
    GROUP BY o.id, s.id
    ORDER BY o.created_at DESC
  `);
