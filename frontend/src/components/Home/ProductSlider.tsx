...
interface ProductSliderProps { title: string; products: Product[]; }
...
const ProductSlider = ({ title, products }: ProductSliderProps) => {
  ...
  const handleAddToCart = (product: Product, e: React.MouseEvent) => {
    ...
    if (product.stock > 0) dispatch(addToCart({ product, quantity: 1 }));
  };

  const isNewProduct = (p: Product) => {
    if (!p.created_at) return false;
    return (Date.now() - new Date(p.created_at).getTime()) / (1000 * 60 * 60 * 24) <= 30;
  };
    ...
    <section className="py-16">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex space-x-2">
          <button onClick={() => scroll("left")} className="p-2 glass-card hover:glow-on-hover animate-smooth"><ChevronLeft className="w-6 h-6 text-primary" /></button>
          <button onClick={() => scroll("right")} className="p-2 glass-card hover:glow-on-hover animate-smooth"><ChevronRight className="w-6 h-6 text-primary" /></button>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mb-2">
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => <Star key={i} className={`w-4 h-4 ${i < Math.floor(product.ratings || 0) ? "text-yellow-400 fill-current" : "text-gray-300"}`} />)}
                </div>
