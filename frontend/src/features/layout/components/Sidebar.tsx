...
import { toggleSidebar } from "../../../app/popupSlice";
...
const Sidebar = () => {
  ...
  const { isSidebarOpen } = useAppSelector((state) => state.popup);
      ...
      <div className="fixed left-0 top-0 h-full w-80 max-w-[85vw] z-50 glass-panel animate-slide-in-left">
        ...
        <nav className="p-6">
          <ul className="space-y-2">
            {menuItems.filter((item): item is NonNullable<typeof item> => item !== null && item !== undefined).map((item) => {
              return (
                <li key={item.name}>
                  <Link
                    to={item.path}
